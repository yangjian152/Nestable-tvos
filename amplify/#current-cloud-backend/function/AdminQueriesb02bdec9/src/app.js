...
app.post('/updateUser', async (req, res, next) => {
  ...
  try {
    const response = await updateUser(req.body.username, req.body.email);
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
...
app.post('/setUserPassword', async (req, res, next) => {
  ...
  try {
    const response = await setPassword(req.body.username, req.body.password);
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
...
app.get('/listUsers', async (req, res, next) => {
  try {
    ...
    let response;
    if (req.query.token) {
      response = await listUsers(q, req.query.limit || 25, req.query.token);
    } else if (req.query.limit) {
      response = await listUsers(q,(Limit = req.query.limit));
    } else {
